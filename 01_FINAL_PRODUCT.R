    ...
    if (input$graph_type == "temp_depth") {
      ggplot(calcofi_filtered, aes(x = depthm, y = t_deg_c)) +
        ...
        labs(
          ...
          x = "Depth (m)",
          y = "Temperature (°C)",
          caption = "Figure 1.) Depth and temperature have an inverse relationship (blue line): as depth increases, water temperature decreases, eventually reaching a minimum of 3.4°C.
          Data from CalCOFI Hydrographic Dataset."
        ) +
        theme_minimal()
    } else {
      ggplot(calcofi_filtered, aes(x = po4u_m, y = o2ml_l)) +
        ...
        labs(
          ...
          x = "Phosphate (μM)",
          y = "Oxygen (ml/L)",
          caption = "Figure 2.) Phosphate and oxygen also have an inverse relationship (blue line): as oxygen increases, phosphate decreases, and vice versa. Data from CalCOFI Hydrographic Dataset."
        ) +
        theme_minimal()
    }
  })
